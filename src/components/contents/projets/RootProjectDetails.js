...
const RootProjectDetails = ({ projectInfo }) => {
  ...
  const projectDetails = useSelector(
    (state) => state.projectDetails[location.key]
  );
          ...
          {local || projectDetails ? (
            <Grid container>
              <Grid item xs={12} md={8}>
                ...
                <Discussion
                  projectId={(local && local.id) || projectDetails.id}
                />
              </Grid>
                  ...
                  <Box height={470} display="flex" flexDirection="column">
                    ...
                    <Box mt={3}>
                      <Typography color="textSecondary">
                        <TwitterIcon className={lClasses.typo} />{" "}
                      </Typography>
                    </Box>
                    <Divider />
                    <Box className={lClasses.paperMaker} mt={3}>
                      <Paper style={{ boxShadow: "none" }}>
                        <HunterMaker
                          author={
                            (local && local.author) || projectDetails.author
                          }
                          authorPhoto={
                            (local && local.authorPhotoUrl) ||
                            projectDetails.authorPhotoUrl ||
                            hack_1
                          }
                          role="Developper"
                          message="Hunter"
                        />
                        {(local && local.isMaker) || projectDetails.isMaker ? (
                          <HunterMaker
                            author={
                              (local && local.author) || projectDetails.author
                            }

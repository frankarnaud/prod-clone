...
export default function ProjectList({ date, connected, prodNumber }) {
  ...
  const projectsData = useSelector((state) => {
    return state.projects.filter(
      (project) => project.date === fromDateToString(date)
    );
  });
  ...
  const loadData = (localDate) => {
    if (!infos) {
      dispatch(
        addProjectListInstance({ prodNumber, pageIndex: 0, isData: false })
      );
  ...
  return (
    <React.Fragment>
      {infos && infos.isData && infos.pageIndex !== 0 ? (
        <Box mt={0} pt={0}>
          <Typography variant="h5" className={classes.typoTitle}>
            {day}
          </Typography>
          <Paper className={classes.paper}>
            ...
            {infos.pageIndex !== projectsData.length ? (
              <>
                <Box
                  {...defaultProps}
                ...
                >
                  ...
                  <Typography
                    ...
                    color="textPrimary"
                  >
                    Show {projectsData.length - infos.pageIndex} more
                  </Typography>
                </Box>
                <Divider />
              </>

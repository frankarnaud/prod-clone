...
const messages = [
  ...
  "Let's make this product look nice",
...
const Submission = ({ user, url, updateProjectInfo }) => {
  ...
  const projectsInfos = useSelector(
    (state) => state.userProjectsInfos.projectsInfos
  );
    ...
    reader.onload = (ev) => {
      if (targetField === "main") {
        if (imageUrls.length !== 0) {
          setImageUrls([
            { file: fileImage, fileBlod: ev.target.result },
            ...imageUrls.slice(1),
          ]);
        } else {
          setImageUrls([
  ...
  const handleNextLaunch = () => {
    let project = {
      ...
      imagesTabs: [...imageUrls.slice(1)],
      isHunter: checkedStatus,
      isMaker: checkedOwner,
      userId: user.uid,
      userRole: user.role || "Developper",
    };
    ...
    if (isOnline) {
      ...
      projectsStore.setItem(name, project).then((storeValue) => {
        if (projectsInfos) {
          dispatch(
            updateUserProjectsInfos("projectsInfos", {
              ...projectsInfos,
              submitted: projectsInfos.submitted + 1,
      ...
      <Container className={classes.container}>
        <Grid container spacing={3}>
          <Grid item xs={6}>
            <Box display="flex">
              <Box pt={3} mr={2}>
                <img style={{ height: "27px" }} src={icon} alt="icon" />
              </Box>{" "}
              <Typography variant="h5" className={classes.typoHeader}>
                {message}
              </Typography>
            </Box>
            <Paper className={classes.paperStyle}>
              {activeStep === 0 ? (
                <Box m={2}>
                      ...
                      InputProps={{
                        endAdornment: (
                          <InputAdornment
                            ...
                            position="end"
                          >{`${name.length}/${MAX_LENGTH}`}</InputAdornment>
                        ),
                      }}
                      inputProps={{ style }}
                  ...
                  <Box>
                    <TextField
                      inputProps={{ style }}
                      placeholder="Concise and descriptive tagline of the product"
                      InputProps={{
                        endAdornment: (
                          <InputAdornment
                            classes={{ root: classes.endAdornmentStyle }}
                      ...
                      onChange={handleChangeSummary.bind(this)}
                      required
                      variant="outlined"
                      fullWidth
                      value={summary}
                  ...
                  <Box>
                    <TextField
                      ...
                      fullWidth
                      value={topic}
                      onChange={handleChangeTopic.bind(this)}
                      inputProps={{ style: { height: "0.1px" } }}
                    >
                      {options.map((option) => (
                        <MenuItem
                          style={{ fontSize: "14px" }}
                  ...
                  <Box mt={2}>
                    ...
                    <Box>
                      <Typography
                        ...
                        color="textSecondary"
                      >
                        Keep it simple. Please no flashy GIFs.
                      </Typography>
                    </Box>
                    <Box mt={2} display="flex">
                      <Avatar
                        variant="square"
                        src={imageUrls[0] && imageUrls[0].fileBlod}
                        ...
                        <Box>
                          <Box mx={2}>
                            <input
                              type="file"
                              accept="image/*"
                              id="upload-file-page-1"
                              className={classes.inputFile}
                              onChange={handleChangeMainImage.bind(this, 1)}
                            />
                            <label
                              htmlFor="upload-file-page-1"
                  ...
                  <CustomCheckbox
                    classStyle={[classes.typoLabel, classes.typoSize]}
                    checked={checkedStatus}
                    handleChange={handleCheckboxChange}
                    messageTitle={`${statusMessages[0].title}`}
                    messageBody={`${statusMessages[0].body}`}
                    index={1}
                  />
                </Box>
              ) : null}
              {activeStep === 1 ? (
                <Box mx={2}>
                  <Box>
                    ...
                    <Box>
                      ...
                      >
                        <Box textAlign="center">
                          <input
                            ...
                            type="file"
                            onChange={handleChangeMainImage.bind(this, 2)}
                          />
                        ...
                        <Box mx={2} textAlign="center">
                          ...
                          <label style={{ color: "grey", fontSize: "15px" }}>
                            {" "}
                            or{" "}
                          </label>
                          <label className={classes.label}>paste URL</label>
                      ...
                      <Box display="flex" mt={1} mb={1} flexWrap="wrap">
                        <Avatar
                          variant="square"
                          className={classes.avatar}
                          src={imageUrls[1] && imageUrls[1].fileBlod}
                        />
                        <Avatar
                          variant="square"
                          className={classes.avatar}
                          src={imageUrls[2] && imageUrls[2].fileBlod}
                        />
                        <Avatar
                          variant="square"
                          className={classes.avatar}
                          src={imageUrls[3] && imageUrls[3].fileBlod}
                        />
                    ...
                    <Box>
                      <TextField
                        ...
                        multiline
                        value={description}
                        onChange={handleChangeArea.bind(this)}
                        variant="outlined"
                  ...
                  <CustomCheckbox
                    ...
                    handleChange={handleCheckboxChange}
                    messageTitle={`${statusMessages[1].title}`}
                    messageBody={`${statusMessages[1].body}`}
                    index={2}
                  />
                </Box>
              ) : null}
                  ...
                  <Box m={2}>
                    ...
                    <Box>
                      <TextField
                        variant="outlined"
                        fullWidth
                        value={productTwitterAccount}
                        placeholder="https://twitter.com/productname"
                        onChange={handleAccountChange.bind(this)}
                        inputProps={{ style }}
                      />
                    </Box>
                    <Box my={2}>
                    ...
                    <Box display="flex">
                      <Box flexGrow={1}>
                        <TextField
                          ...
                          value={marker}
                          inputProps={{ style }}
                          onChange={handleMakerChange.bind(this)}
                        />
                      </Box>
                      <Box ml={1}>
              ...
              <Box display="flex" pb={2}>
                <Box flexGrow={1}>
                  <IconButton
                    onClick={handleButtonClick.bind(this, 0)}
                    className={clsx(classes.iconBtn, classes.iconBtnDisabled)}
                    disabled={activeStep === 0}
                  >
                    <LensIcon className={classes.iconSize} />
                  </IconButton>
                  <IconButton
                ...
                <Box mx={2}>
                  {activeStep === 0 ? (
                    <Button
                      disableRipple
                      className={clsx(classes.btn, classes.btnDisabled)}
                      onClick={handlePageImage}
                      disabled={enablePageImageButton()}
                    >
                      next: images
                    </Button>
                  ) : null}
                  ...
                  {activeStep === 2 ? (
                    <Button
                      variant="outlined"
                      style={{ fontSize: "12spx", marginRight: "5px" }}
                    >
                      schedule launch
                    </Button>
                  ) : null}

...
const VoteCounter = ({projectId, user, counter, details, updateDialog}) => {
    ...
    const [styled, setStyled] = React.useState((voteInfos && voteInfos.voted) || false)
    ...
    const listenerChange = (projectId) => {
       const vRef = firestore.collection('projects').doc(projectId).collection('voted_users')
    ...
    const updateCounter = () => {
        ...
        const vRef = firestore.collection('projects').doc(projectId).collection('voted_users').doc(user.uid)
        if(!voted){
    ...
    React.useEffect(() => {
        if(user && !voteInfos){
            const prodVoteRef = firestore.collection('users').doc(user.uid || 'hacker').collection('projects_votes').doc(projectId)
    ...
    if(!details){
        return (
            <Box className={clsx(classes.counter, (user && styled)?classes.votedStyle: '')} onClick={handleClick}>
                <ArrowDropUpIcon />
                <Typography variant="h6" style={{fontSize: '14px'}}>{localCounter}</Typography>
            </Box>
        )
    }
    else {
        return (
        <Box className={clsx(classes.counterDetails,(user && styled)?classes.votedStyle: '')} onClick={handleClick}>
                <Typography><ArrowDropUpIcon /></Typography>
                <Typography variant="h6" style={{fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase'}}>UpVote {localCounter}</Typography>
            </Box>
        )
    }   
}

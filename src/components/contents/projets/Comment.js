        ...
        <Box ml={3} pl={3}>
          <Box>
            {comment.message.split("\n").map((cmnt, index) => (
              <Typography className={classes.typoComment} key={index}>
                {cmnt}
              </Typography>
            ))}
          </Box>
          <Box display="flex" mt={1}>
            <Box
              mx={1}
              onClick={handleUpVoteClick.bind(this, null)}
              className={classes.box}
            >
              <Typography
                variant="h6"
                color="textSecondary"
                className={classes.typoMenuComment}
              >
            ...
            <Box
              mx={1}
              onClick={handleReplyClick.bind(this)}
              className={classes.box}
            >
              <Typography
                variant="h6"
                ...
                className={classes.typoMenuComment}
              >
                {counter || comment.replyCounter} reply
              </Typography>
            </Box>
            <Box
              mx={1}
              onClick={handleShareClick.bind(this, null)}
              className={classes.box}
            >
              <Typography
                variant="h6"
            ...
            <Box display="flex">
              ...
              <Box ml={1}>
                <Button
                  ...
                  onClick={replyComment.bind(this)}
                >
                  reply
                </Button>
              </Box>
        ...
        {replies.length !== 0 ? (
          <List dense component="nav">
            {replies.map((reply, index) => (
              <ListItem
                style={{ display: "block", paddingLeft: "5.5%" }}
                key={index}
              >
                <Box display="flex" flexGrow={1}>
                  <ListItemAvatar style={{ paddingBottom: "10px" }}>
                    <Avatar
                      className={classes.avatar}
